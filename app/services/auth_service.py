...
def login_user(db: Session, user, client_id: str = None):
    ...
    expires_at = datetime.utcnow() + timedelta(seconds=REFRESH_TOKEN_EXPIRES_SECONDS)
...
def refresh_access_token(db: Session, refresh_token_plain: str, client_id: str = None):
    ...
    new_expires_at = datetime.utcnow() + timedelta(seconds=REFRESH_TOKEN_EXPIRES_SECONDS)
